...
func (c *VcapComponent) ListenAndServe() {
	...
	brokerConfig := c.Config.(*config.Config).Broker
	serviceBroker := &servicebroker.ServiceBroker{&brokerConfig}
	...
	f := func(user, password string) bool {
		return (user == c.Varz.Credentials[0] && password == c.Varz.Credentials[1]) ||
			(user == brokerCredentials.Username && password == brokerCredentials.Password)
	}
