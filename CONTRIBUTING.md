...
# Contributing Guide for cf-bigip-ctlr
...
* Include version of Cloud Foundry, cf-bigip-ctlr, and BIG-IP
...
## Pull Requests
We use [travis-ci](https://travis-ci.org/F5Networks/cf-bigip-ctlr) to automatically run some hooks on every pull request. These must pass before a pull request is accepted. You can see the details in [.travis.yml](https://github.com/F5Networks/cf-bigip-ctlr/blob/master/.travis.yml). If your pull request fails in travis, your pull request will be blocked by travis with a link to the failing run. Generally, we run these hooks:
